...
def get_account_state_query(address: str, tag: Tag) -> Query:
    return select(get_default_fields()).where(
        and_(
            ...
            accounts_state_t.c.block_number <= select([blocks_t.c.number]).where(blocks_t.c.hash == tag.value),
        )
    )
    ...
    query = select(get_default_fields()).where(accounts_state_t.c.address == address)
