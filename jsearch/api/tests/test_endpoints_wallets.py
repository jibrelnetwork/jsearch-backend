...
@pytest.mark.parametrize(
    "block_number, event_index, status",
    (
            ('7850000', '78500000060010', 200),
            ('7850000', '10', 400),
            ('7850000', 'not-an-int', 400),
            ('not-an-int', '10', 400),
            (None, None, 200),
            (None, '78500000060010', 200),
            ('7850000', None, 200),
    ),
    ids=(
            "event index relates to block number",
            "event index does not relate to block number",
            "string inside event index",
            "string inside block number",
            "event index is empty",
            "block number is empty",
            "event index and block number are empty",
    )
)
async def test_get_wallet_events_filter_by_block_number_and_event_index(
        cli: TestClient,
        block_number: str,
        event_index: str,
        status: int
    ...
    params = urlencode({k: v for k, v in params_dict.items() if v is not None})
    url = URL.format(params=params)
