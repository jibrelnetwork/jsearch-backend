...
class TransactionCase(NamedTuple):
    ...
    def to_tuple(self):
        return self.data, self.is_pending, self.expected_type
...
@pytest.mark.parametrize(
    "data,is_pending,expected",
    argvalues=[x.to_tuple() for x in txs_cases],
    ids=[x.id for x in txs_cases],
)
def test_find_event_type(data: Transaction, is_pending: bool, expected: str) -> None:
    # when
    result = get_event_type(data, is_pending=is_pending)
