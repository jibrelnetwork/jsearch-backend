...
Revision ID: 99dd806561de
Revises: b1863b60ea3c
Create Date: 2019-08-19 12:25:40.372621

"""
from alembic import op

# revision identifiers, used by Alembic.
revision = '99dd806561de'
...
BEGIN
	...
	IF json_array_length(token_holders_updates_data::json) > 0 THEN
		INSERT INTO token_holders SELECT account_address, token_address, balance, decimals, block_number FROM json_populate_recordset(null::token_holders, token_holders_updates_data::json)
            ON CONFLICT (account_address, token_address) DO UPDATE SET balance = EXCLUDED.balance, block_number = EXCLUDED.block_number
		    WHERE token_holders.block_number < EXCLUDED.block_number;
	END IF;
...
BEGIN
	...
	IF json_array_length(token_holders_updates_data::json) > 0 THEN
		INSERT INTO token_holders SELECT token_address, account_address, balance, decimals, block_number FROM json_populate_recordset(null::token_holders, token_holders_updates_data::json)
            ON CONFLICT (account_address, token_address) DO UPDATE SET balance = EXCLUDED.balance, block_number = EXCLUDED.block_number
		    WHERE token_holders.block_number < EXCLUDED.block_number;
	END IF;
