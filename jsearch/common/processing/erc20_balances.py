...
def get_data(owner) -> str:
    address = Web3.toChecksumAddress(owner)
    data = to_hex(HexBytes('0x70a08231') + encode_abi(abi, [address]))
    return data


def get_params(contract: str,
               data: str,
               block: Union[int, str] = LATEST_BLOCK) -> List[Any]:
    if block is None:
        block = LATEST_BLOCK
...
def get_params_balance(contract: str, owner: str, block: Optional[int] = None) -> List[Any]:
    return get_params(contract, data=get_data(owner), block=block)


def get_balance_rpc_call(contract: str, owner: str, _id: int, block: Optional[int] = None):
    value = {
        "jsonrpc": "2.0",
        "method": 'eth_call',
        "params": get_params(contract, data=get_data(owner), block=block),
        "id": _id,
    }
    return value
...
async def eth_call(call):
    response = await eth_call_request(call)
    return {result["id"]: HexBytes(result["result"]) for result in response}
