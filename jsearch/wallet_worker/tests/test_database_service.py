...
async def test_add_wallet_event_token_transfer(db, transfer_factory, transaction_factory):
    s = DatabaseService()
    await s.on_start()
    ...
    assert dict(res[0]) == {'address': transfer_data['from_address'],
                            ...
                            'event_data': {'recipient': transfer_data['to_address'],
...
async def test_add_wallet_event_tx(db, transaction_factory):
    s = DatabaseService()
    await s.on_start()
    ...
    await s.add_wallet_event_tx(tx_data)

    res = db.execute(wallet_events_t.select()).fetchall()
    assert len(res) == 2
    assert dict(res[0]) == {'address': tx_data['from'],
                            'block_hash': tx_data['block_hash'],
                            'block_number': tx_data['block_number'],
                            'event_data': {'recipient': tx_data['to'],
                                           'sender': tx_data['from'],
                                           'amount': '65535',
                                           'status': 1},
                            'event_index': 1000 * tx_data['block_number'] + tx_data['transaction_index'],
                            ...
                            'tx_hash': tx_data['hash'],
                            'type': 'eth-transfer'}
    assert dict(res[1]) == {'address': tx_data['to'],
                            'block_hash': tx_data['block_hash'],
                            'block_number': tx_data['block_number'],
                            'event_data': {'recipient': tx_data['to'],
                                           'sender': tx_data['from'],
...
async def test_add_wallet_event_tx_internal(db, transaction_factory):
    s = DatabaseService()
    await s.on_start()
    tx = transaction_factory.create(input='0x', value='0xffff')
    tx_data = tx.to_dict()
    ...
    await s.add_wallet_event_tx_internal(tx_data, internal_tx_data)

    res = db.execute(wallet_events_t.select()).fetchall()
    assert len(res) == 2
    assert dict(res[0]) == {'address': internal_tx_data['from'],
                            'block_hash': tx_data['block_hash'],
                            'block_number': tx_data['block_number'],
                            'event_data': {'recipient': internal_tx_data['to'],
                            ...
                            'tx_hash': tx_data['hash'],
                            'type': 'eth-transfer'}
    assert dict(res[1]) == {'address': internal_tx_data['to'],
                            'block_hash': tx_data['block_hash'],
                            'block_number': tx_data['block_number'],
                            'event_data': {'recipient': internal_tx_data['to'],
                                           'sender': internal_tx_data['from'],
